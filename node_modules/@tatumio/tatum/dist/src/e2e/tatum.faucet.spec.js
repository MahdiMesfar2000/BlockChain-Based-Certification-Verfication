...
describe('Tatum faucet', () => {
    ...
    describe('invalid request', () => {
        ...
        it('should get error due to unsupported chain', async () => {
            const tatum = await service_1.TatumSDK.init({
                network: service_1.Network.MULTIVERSX,
            });
    ...
    describe('valid request', () => {
        it.skip('should only stop at balance being above limit', async () => {
            const tatum = await service_1.TatumSDK.init({
                network: service_1.Network.POLYGON_MUMBAI,
            });
        ...
        it.skip('should return success', async () => {
            const tatum = await service_1.TatumSDK.init({
                ...
                apiKey: {
                    v4: process_1.default.env.V4_API_KEY_TESTNET,
                },
            });
            const res = await tatum.faucet.fund(SEPOLIA_VAULT);
            await tatum.destroy();
            expect(res.status).toBe(util_1.Status.SUCCESS);
        });
    });
});
//# sourceMappingURL=tatum.faucet.spec.js.map